...
- **Logging** — `from app.core.logging import get_logger`, then `log.exception(...)`
  in `except` blocks. `configure_logging()` (idempotent) sets up console +
  `logs/app.log` + `logs/errors.log`; the latter collects `WARNING`+ with
  tracebacks from the API and the workers. Unhandled request errors and Celery
  `task_failure` are captured centrally — you don't have to try/except every
  handler, but do log-and-reraise where you can add context. Both files stamp a
  day-separator banner (`--------------------------=11-Sep-25----...`) before
  each calendar day's first line — see CONFIGURATION.md § Logging.
